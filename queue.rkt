...
; Definiți valoarea care reprezintă o structură queue goală.
  (define empty-queue (make-queue empty-stream '() 0 0))
...
(define (rotate left right Acc)
  (if (stream-empty? left)
      (if (null? right)
          Acc
       (stream-cons (car right) (rotate left (cdr right) Acc)))
...
(define (enqueue x q)
  (if (and (stream-empty? (queue-left q)) (null? (queue-right q))) (struct-copy queue q [left (stream-cons x (queue-left q))] [size-l (+ (queue-size-l q) 1)])
  (if (= (queue-size-r q) (queue-size-l q)) (struct-copy queue q
                                                         [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)]
                                                         [right '()] [size-l (+ (queue-size-l q) (+ (queue-size-r q) 1))] [size-r 0])
  (struct-copy queue q [right (append (list x) (queue-right q))] [size-r (+ (queue-size-r q) 1)]))))
...
(define (dequeue q)
  (if (= (queue-size-l q) (queue-size-r q)) (struct-copy queue q [left (stream-rest (rotate (queue-left q) (queue-right q) empty-stream))]
                                                         [right '()] [size-l (- (+ (queue-size-l q) (queue-size-r q)) 1)] [size-r 0])
